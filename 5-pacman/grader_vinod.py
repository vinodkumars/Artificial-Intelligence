...
def test0(agentName):
  ...
  if agentName == 'alphabeta':
    ...
    stats = run('smallClassic', submission.ExpectimaxAgent(evalFn='better',depth=2), [DirectionalGhost(i + 1) for i in range(2)], name='%s(depth %d)' % ('BetterExpectimax', 2))
  if stats['timeouts'] > 0:
...
grader.addBasicPart('4-0', lambda : test0('betterExpectimax'), 4, description='Tests betterExpectimax for timeout on smallClassic.')
