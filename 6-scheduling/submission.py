...
def create_chain_csp(n):
    ...
    for i in variables:
        ...
        if prev != None:
            csp.add_binary_potential(prev, i, lambda a, b: a != b)
        prev = i
...
def create_nqueens_csp(n=8):
    ...
    def condition(a, b):
        return a[1] != b[1] and abs(a[1] - b[1]) != abs(a[0] - b[0])

    for i in range(n):
        for j in range(i + 1, n):
            csp.add_binary_potential(i, j, condition)
...
class BacktrackingSearch:
    ...
    def get_delta_weight(
        ...
        if self.csp.unaryPotentials[var]:
            ...
            if w == 0:
                return w
        for (var2, potential) in \
            self.csp.binaryPotentials[var].iteritems():
            if assignment[var2] == None:  # Not assigned yet
                continue
    ...
    def solve(
        ...
        self.domains = [list(range(len(domain))) for domain in
                        self.csp.valNames]
    ...
    def backtrack(
        ...
        if numAssigned == self.csp.numVars:
            ...
            for var in range(self.csp.numVars):
                newAssignment[self.csp.varNames[var]] = \
        ...
        if not self.mac:
            ...
            for val in ordered_values:
                deltaWeight = self.get_delta_weight(assignment, var,
                        val)
                if deltaWeight > 0:
                    assignment[var] = val
    ...
    def arc_consistency_check(self, var):
        ...
        while len(queue) != 0:
            ...
            for neighborVar in self.csp.get_neighbor_vars(currentVar):
                ...
                if self.csp.unaryPotentials[neighborVar] != None:
                    for neighborVal in (self.domains[neighborVar])[:]:
                        if self.csp.unaryPotentials[neighborVar][neighborVal] \
                            == 0:
                            self.domains[neighborVar].remove(neighborVal)
                            isDomainChanged = True

                if self.csp.binaryPotentials[currentVar][neighborVar] \
                    != None:
                    for neighborVal in (self.domains[neighborVar])[:]:
                        inconsistentCount = 0
                        for currentVal in self.domains[currentVar]:
                            if self.csp.binaryPotentials[currentVar][neighborVar][currentVal][neighborVal] \
                                == 0:
                                inconsistentCount += 1
                        if len(self.domains[currentVar]) \
                            == inconsistentCount:
                            self.domains[neighborVar].remove(neighborVal)
                            isDomainChanged = True
...
def get_or_variable(
    ...
    for (i, X_i) in enumerate(variables):
        ...
        if i == 0:
            ...
            csp.add_binary_potential(('or', name, i - 1), A_i,
                    potential)

    # consistency between A_n and result
    # hacky: reuse A_i because of python's loose scope

    csp.add_binary_potential(A_i, result, lambda val, res: res == (val
                             != 'no'))
    return result


def get_sum_variable(
    csp,
    name,
    variables,
    maxSum,
    ...
    for (i, X_i) in enumerate(variables):
        A_i = ('sum', name, i)
        csp.add_variable(A_i, [(r, s) for r in range(maxSum + 1)
        ...
        if i == 0:
            ...
            csp.add_binary_potential(('sum', name, i - 1), A_i,
                    potential)

    csp.add_binary_potential(A_i, result, lambda a, res: res == a[1])
    return result
...
class SchedulingCSPConstructor:
    ...
    def add_bulletin_constraints(self, csp):
        ...
        for req in self.profile.requests:
            for quarter in self.profile.quarters:
                csp.add_unary_potential((req, quarter), lambda cid: cid \
                        is None \
            ...
            for quarter1 in self.profile.quarters:
                for quarter2 in self.profile.quarters:
                    if quarter1 == quarter2:
                        continue
                    csp.add_binary_potential((req, quarter1), (req,
                            quarter2), lambda cid1, cid2: cid1 is None \
                            or cid2 is None)
    ...
    def add_quarter_constraints(self, csp):
        ...
        for req in self.profile.requests:
            for quarter in self.profile.quarters:
                csp.add_unary_potential((req, quarter), lambda cid: cid \
                        == None or len(req.quarters)==0 or quarter in req.quarters)
    ...
    def add_prereq_constraints(self, csp):
        ...
        for req in self.profile.requests:
            ...
            for (quarter_i, quarter) in \
                enumerate(self.profile.quarters):
                ...
                for pre_cid in req.prereqs:
                    ...
                    for pre_req in self.profile.requests:
                        ...
                        prereq_vars = [(pre_req, q) for (i, q) in
                                enumerate(self.profile.quarters) if i
                                < quarter_i]
                        v = (req, quarter)
                        orVar = get_or_variable(csp, (v, pre_cid),
                                prereq_vars, pre_cid)

                        # Note this constraint is enforced only when the course is taken
                        # in `quarter` (that's why we test `not val`)

                        csp.add_binary_potential(orVar, v, lambda o, \
                                val: not val or o)
...
# acSolver.solve(create_nqueens_csp(8), mac=True)
# print acSolver.numOptimalAssignments
# print acSolver.firstAssignmentNumOperations
